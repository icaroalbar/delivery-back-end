...
import { RefuseFoodUseCase } from "./refuse-food.usecase";
...
describe("RefuseFoodUseCase", () => {
  let refuseFoodUseCase: RefuseFoodUseCase;

  beforeEach(() => {
    jest.clearAllMocks();
    refuseFoodUseCase = new RefuseFoodUseCase(mockFoodRepository);
  });

  it("Deve recusar o pedido", async () => {
    mockFoodRepository.findById.mockResolvedValue(mockFood);
    const result = await refuseFoodUseCase.execute(mockFood.getId());
